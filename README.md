...
- **Incremental reveal, in one selectable pane.** `Show next step` releases exactly one further line of the derivation. `Show answer` releases the answer. Once the answer is out there is nothing left to protect, so a step request then returns the complete worked solution at once — every revealed step followed by the answer, all rendered together in one pane, so the whole derivation can be selected and copied in a single drag.
...
- **24 derivative templates** across power, product, quotient, chain, trigonometric, exponential/logarithmic, and partial-derivative rules — every one of them verified against SymPy before it ships (see [Correctness](#correctness)). Product- and quotient-rule answers that don't reduce to a closed form by simplification alone (a distributed numerator, or a second factor multiplied through) are expanded and collected, not left half-factored — the `Substitute` step still shows the raw `g'h + gh'` form a person writes by hand, and a following `Expand and collect` step reaches the single polynomial the app treats as the answer. The partial-derivative sub-type covers first-order `∂/∂x` and `∂/∂y` in two variables across four families — polynomial, product-of-factors, exponential, and trig — each holding the other variable constant.
...
- **Flag a problem as Key, and note any problem whether or not it's starred.** A star toggle in Practice — and a matching one on any past problem in Sessions, both sized for a quick tap — marks it Key. A Note field sits alongside every problem in Practice and Sessions, starred or not; un-starring a problem keeps its note rather than discarding it. Sessions and Practice both show every entry as a single compact line: topic · sub-type, level, ID, the Key star, right/wrong/skip, the typeset equation itself, and the time, with a `+` button that pops up just that equation as copyable Markdown. The **Key** tab lists every starred problem with its prompt and note for quick review; tapping its star to unstar only stages the removal — dimmed until you leave the tab — so an accidental unstar is easy to undo, and each row has its own equation-only Markdown popup alongside an "Export PDF" button that turns every starred problem (with its note) into one PDF.
- **View any solution as Markdown.** A "View as Markdown" button in the Reveal section pops up the current problem, its instruction, and whatever's been revealed — steps, then the answer — as raw, selectable Markdown source (maths wrapped in single `$...$` delimiters), plus a one-tap Copy button that puts the same text on the clipboard, ready to paste into Obsidian or any other Markdown-aware app.
...
A template's *prose* is checked separately, because a correct derivative posed as the wrong question is still a wrong problem — and SymPy has no opinion about English. Each template's instruction is asserted against its own `verificationRule`, the enum that decides what counts as a correct answer, so the question and the marking scheme cannot drift apart silently: a `∂/∂x` template must ask for `∂f/∂x` and must not name `y` as the variable it is differentiated by. Step titles are checked too — every title of every template, across its whole difficulty band, must keep any LaTeX inside balanced `$…$` delimiters, so nothing reaches the screen as raw markup.
...
Sources/MathPracticeCore/     pure domain logic — no SwiftUI, WebKit, SwiftData, or I/O
  ...
  Topics/Derivatives/         topic 1: 24 templates across 7 sub-types
...
Step labels are typeset, not just the expressions beside them. A step that names the term it acts on ("Differentiate $x^{3}y^{3}$") delimits that maths with the same single `$` the Markdown view uses, and the renderer typesets those spans as part of the same page — so one convention covers the screen, the PDF exports and the copyable Markdown.
